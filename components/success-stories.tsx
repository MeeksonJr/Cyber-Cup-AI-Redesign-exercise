  ...
  return (
    <section id="success-stories" className="py-24 md:py-32 bg-gradient-to-b from-white to-[#f0f4f8] relative overflow-hidden" ref={ref}>
      ...
      <div className="absolute inset-0 opacity-5">
        <div className="absolute top-20 left-10 w-72 h-72 bg-[#043657] rounded-full blur-[100px]" />
        <div className="absolute bottom-20 right-10 w-96 h-96 bg-[#00b4d8] rounded-full blur-[120px]" />
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#98C5EA]/20 border border-[#043657]/20 mb-4">
            <Star className="w-5 h-5 text-[#043657]" />
            <span className="text-sm font-semibold text-[#043657] uppercase tracking-wider">Success Stories</span>
          </div>
          <h2 className="font-display text-4xl md:text-5xl lg:text-6xl font-bold text-[#043657] mb-4">
            Champions <span className="text-[#00b4d8]">Speak</span>
          </h2>
        ...
        <div className="grid md:grid-cols-3 gap-8 mb-12">
          {stories.map((story, index) => (
            <motion.div
              ...
              className="group"
            >
              <div className="bg-white rounded-[0.625rem] border border-[#828A8F]/20 p-8 h-full flex flex-col hover:shadow-xl transition-all duration-300 hover:border-[#00b4d8]/40">
                {/* Quote Icon */}
                <div className="mb-6">
                  <div className="inline-flex items-center justify-center w-12 h-12 rounded-lg bg-[#98C5EA]/20">
                    <Quote className="w-6 h-6 text-[#043657]" />
                  </div>
                </div>

                {/* Quote */}
                <blockquote className="text-[#043657] text-lg leading-relaxed mb-6 flex-grow italic">
                  "{story.quote}"
                </blockquote>

                {/* Author Info */}
                <div className="border-t border-[#828A8F]/10 pt-6">
                  <div className="flex items-center gap-4 mb-4">
                    <div className="w-16 h-16 rounded-full bg-gradient-to-br from-[#043657] to-[#1a4b8c] flex items-center justify-center text-white text-2xl font-bold">
                      {story.image}
                    </div>
                    <div>
                      <div className="font-bold text-[#043657] text-lg">{story.name}</div>
                      <div className="text-sm text-[#828a8f]">{story.company}</div>
                    </div>
                  </div>

                  {/* Achievement Badge */}
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#98C5EA]/20 border border-[#043657]/20">
                      <Award className="w-4 h-4 text-[#043657]" />
                      <span className="text-xs font-semibold text-[#043657]">{story.achievement}</span>
                    </div>
                    <div className="text-right">
                      <div className="text-sm font-bold text-[#00b4d8]">#{story.stats.rank}</div>
                      <div className="text-xs text-[#828a8f]">{story.stats.score} pts</div>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <motion.div
          ...
          className="bg-gradient-to-r from-[#043657] to-[#1a4b8c] rounded-[0.625rem] p-8 md:p-12 text-center text-white"
        >
          <h3 className="font-display text-2xl md:text-3xl font-bold mb-4">Ready to Write Your Success Story?</h3>
          <p className="text-[#98c5ea] max-w-2xl mx-auto mb-8">
